...
export const pipelineRunsReferenceTime = Date.parse("2026-08-10T13:30:00.000Z");
...
export const pipelineRuns: PipelineRun[] = [
    { id: "run_01J91CPM41", pipelineId: "customer-profile-merge", pipelineName: "Customer Profile Merge", status: "Running", stage: "Transform", trigger: "Scheduled", startedAt: minutesAgo(3.7), platformCode: "PIPELINE_RUNNING", message: "The pipeline is currently transforming customer profiles.", recommendedAction: "No action is required while execution continues." },
    { id: "run_01J92CING8", pipelineId: "customer-ingestion", pipelineName: "Customer Ingestion", status: "Success", stage: "Load", trigger: "Scheduled", startedAt: minutesAgo(8), durationSeconds: 134, records: 124892, platformCode: "RUN_COMPLETED", message: "The pipeline completed successfully.", recommendedAction: "No action is required." },
    ...
    { id: "run_01J94EVT18", pipelineId: "events-processing", pipelineName: "Events Processing", status: "Failed", stage: "Extract", trigger: "Event", startedAt: minutesAgo(28), durationSeconds: 34, platformCode: "PIPELINE_EXECUTION_FAILED", vendorCode: "SASL_AUTHENTICATION_FAILED", message: "Authentication to the event broker failed during extraction.", recommendedAction: "Verify broker credentials and retry the run." },
    ...
    { id: "run_01J97BIL02", pipelineId: "billing-reconciliation", pipelineName: "Billing Reconciliation", status: "Failed", stage: "Validate", trigger: "Scheduled", startedAt: minutesAgo(122), durationSeconds: 522, records: 118204, platformCode: "VALIDATION_CHECK_FAILED", vendorCode: "CHECK_NULL_RATE_THRESHOLD", message: "The null-rate threshold was exceeded during validation.", recommendedAction: "Review rejected billing records before retrying the run." },
    ...
    { id: "run_01JA7OLD40", pipelineId: "customer-ingestion", pipelineName: "Customer Ingestion", status: "Failed", stage: "Extract", trigger: "Scheduled", startedAt: minutesAgo(40 * 1440), durationSeconds: 18, platformCode: "PIPELINE_EXECUTION_FAILED", vendorCode: "SQLSTATE 08006", message: "The database connection was interrupted during extraction.", recommendedAction: "Verify source connectivity and retry the run." },
];
