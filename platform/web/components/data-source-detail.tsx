...
export function DataSourceDetailPage({ source }: { source: DataSourceDetail }) {
    ...
    const actions = <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto"><Button onClick={() => router.push(`/validation?${new URLSearchParams({ source: source.id, environment: source.environment }).toString()}`)}>View Validation</Button><Button onClick={() => router.push(`/monitoring?${new URLSearchParams({ resourceType: "Data Sources", environment: source.environment, resource: source.id }).toString()}`)}>View Monitoring</Button><Button onClick={() => router.push(`/logs?${new URLSearchParams({ scope: "source", source: source.id, environment: source.environment, ...(source.health.platformCode ? { code: source.health.platformCode } : {}), time: "1h" }).toString()}`)}><FileText className="h-3.5 w-3.5" /> View Logs</Button><Button variant="primary" onClick={() => showNotice(source.status === "Disconnected" ? `Connection test failed · ${source.health.platformCode}${source.health.vendorCode ? ` · ${source.health.vendorCode}` : ""}` : source.status === "Disabled" ? "Connection test unavailable while this source is disabled." : `Connection test completed · ${source.health.platformCode} · ${source.health.latency}`)}><PlugZap className="h-3.5 w-3.5" /> Test Connection</Button><Button disabled title="Source editing is not available in this MVP"><Pencil className="h-3.5 w-3.5" /> Edit</Button></div>;
