...
export function PipelineRunsPage() {
    ...
    const clearFilters = () => { setQuery(""); setStatus("All"); setPipelineId("All"); setTrigger("All"); setTimeRange("day"); };
    ...
    return <div className="animate-enter"><PageHeader title="Pipeline Runs" description="View and investigate pipeline execution history." />{summaryError ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="rounded-lg border border-zinc-200 bg-white p-4 shadow-card"><AlertTriangle className="h-4 w-4 text-amber-500" /><p className="mt-3 text-xs font-medium text-zinc-700">Summary unavailable</p><p className="mt-1 font-mono text-[10px] text-zinc-500">PIPELINE_RUN_SUMMARY_UNAVAILABLE</p><button onClick={() => setSummaryError(false)} className="mt-2 text-[11px] font-medium text-indigo-600 hover:text-indigo-700">Try Again</button></div>)}</div> : <RunMetrics runs={rangeRuns} rangeLabel={selectedRange.label} />}<section className="mt-7"><div className="mb-3 flex flex-col gap-2 xl:flex-row xl:items-center"><SearchField value={query} onChange={setQuery} placeholder="Search runs..." className="xl:max-w-xs" /><FilterSelect label="Filter by status" value={status} onChange={(value) => setStatus(value as PipelineRunStatus | "All")} options={[{ label: "All statuses", value: "All" }, ...(["Running", "Success", "Failed", "Cancelled"] as PipelineRunStatus[]).map((value) => ({ label: value, value }))]} /><FilterSelect label="Filter by pipeline" value={pipelineId} onChange={setPipelineId} options={[{ label: "All pipelines", value: "All" }, ...pipelines.map((pipeline) => ({ label: pipeline.name, value: pipeline.id }))]} /><FilterSelect label="Filter by trigger" value={trigger} onChange={(value) => setTrigger(value as PipelineRunTrigger | "All")} options={[{ label: "All triggers", value: "All" }, ...(["Scheduled", "Manual", "Retry", "Event"] as PipelineRunTrigger[]).map((value) => ({ label: value, value }))]} /><FilterSelect label="Filter by time range" value={timeRange} onChange={(value) => setTimeRange(value as PipelineRunTimeRange)} options={timeRangeOptions.map(({ label, value }) => ({ label, value }))} /><span className="text-[11px] text-zinc-400 xl:ml-auto">{filteredRuns.length} {filteredRuns.length === 1 ? "run" : "runs"}</span></div>{supportingError && <SupportingDataWarning onRetry={() => setSupportingError(false)} />}{!runs.length ? <EmptyState title="No pipeline runs yet" description="Pipeline executions will appear here after a pipeline runs." icon={<Activity className="h-4 w-4" />} tone="neutral" action={<Button onClick={() => router.push("/pipelines")}>View Pipelines</Button>} /> : !filteredRuns.length ? <EmptyState title="No runs match your filters" description="Try adjusting your search, status, pipeline, trigger, or time range." icon={<Search className="h-4 w-4" />} tone="neutral" action={<Button onClick={clearFilters}>Clear Filters</Button>} /> : <PipelineRunsTable runs={filteredRuns} now={now} activeMenu={activeMenu} onMenuChange={setActiveMenu} onOpen={() => placeholder("Pipeline Run detail is not available in this phase.")} onRetry={(run) => setDialog({ kind: "retry", run })} onCancel={(run) => setDialog({ kind: "cancel", run })} onViewPipeline={(run) => router.push(`/pipelines/${run.pipelineId}`)} onViewLogs={() => placeholder("Logs are not available in this phase.")} />}</section><div className="sr-only"><button onClick={() => setPageError(true)}>Show pipeline runs error</button><button onClick={() => setSupportingError(true)}>Show pipeline reference data error</button><button onClick={() => setSummaryError(true)}>Show pipeline run summary error</button><button onClick={() => setRuns([])}>Show empty pipeline runs</button><button onClick={() => setRetryError(true)}>Make next retry fail</button></div><ConfirmationDialog open={Boolean(dialog)} title={dialog?.kind === "cancel" ? "Cancel this pipeline run?" : "Retry this pipeline run?"} description={dialog?.kind === "cancel" ? "The current execution will be stopped. Any work already committed by the pipeline may remain." : "This will start a new execution using the pipeline's current configuration."} details={dialog?.kind === "retry" ? [{ label: "Pipeline", value: dialog.run.pipelineName }, { label: "Original Run", value: dialog.run.id }] : undefined} cancelLabel={dialog?.kind === "cancel" ? "Keep Running" : "Cancel"} confirmLabel={dialog?.kind === "cancel" ? "Cancel Run" : "Retry Run"} confirmVariant={dialog?.kind === "cancel" ? "danger" : "primary"} onCancel={() => setDialog(null)} onConfirm={confirmAction} />{retryError && !dialog && <div className="fixed inset-0 z-50 grid place-items-center bg-zinc-950/30 p-4 backdrop-blur-[2px]" onMouseDown={(event) => { if (event.target === event.currentTarget) setRetryError(false); }}><div className="w-full max-w-md"><ErrorState title="Pipeline retry could not be started" description="The orchestration service did not accept the retry request." actionLabel="Try Again" technicalDetails={[{ label: "Platform Code", value: "PIPELINE_RETRY_FAILED" }, { label: "Vendor Code", value: "HTTP 503" }]} onRetry={() => setRetryError(false)} /></div></div>}{toast && <Toast {...toast} onClose={() => setToast(null)} />}</div>;
}
