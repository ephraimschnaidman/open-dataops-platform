...
export function ConfirmationDialog({ open, title, description, confirmLabel, cancelLabel = "Cancel", onCancel, onConfirm, confirmVariant = "primary", supportingText, details }: { open: boolean; title: string; description: string; confirmLabel: string; cancelLabel?: string; onCancel: () => void; onConfirm: () => void; confirmVariant?: "primary" | "danger"; supportingText?: string; details?: Array<{ label: string; value: string }> }) {
    ...
    if (!open) return null;
    return <div role="presentation" className="fixed inset-0 z-50 grid place-items-center bg-zinc-950/30 p-4 backdrop-blur-[2px]" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}><div role="dialog" aria-modal="true" aria-labelledby="confirmation-title" className="animate-enter w-full max-w-sm rounded-xl border border-zinc-200 bg-white p-5 shadow-panel"><h2 id="confirmation-title" className="text-base font-semibold tracking-[-0.02em] text-zinc-900">{title}</h2><p className="mt-2 text-sm leading-6 text-zinc-500">{description}</p>{supportingText && <p className="mt-2 text-xs leading-5 text-zinc-400">{supportingText}</p>}{details && <dl className="mt-4 grid gap-2 rounded-md bg-zinc-50 p-3 text-xs">{details.map((detail) => <div key={detail.label} className="grid grid-cols-[88px_1fr] gap-2"><dt className="text-zinc-500">{detail.label}</dt><dd className="truncate font-medium text-zinc-800" title={detail.value}>{detail.value}</dd></div>)}</dl>}<div className="mt-5 flex justify-end gap-2"><Button onClick={onCancel}>{cancelLabel}</Button><Button variant={confirmVariant} onClick={onConfirm}>{confirmLabel}</Button></div></div></div>;
}
