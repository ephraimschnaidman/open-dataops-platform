...
const navGroups: NavGroup[] = [
    ...
    { label: "Operate", items: [{ name: "Pipeline Runs", icon: Activity, href: "/pipeline-runs" }, { name: "Monitoring", icon: MonitorDot, href: "/monitoring" }, { name: "Alerts", icon: ShieldAlert, count: 4 }, { name: "Logs", icon: FileText }] },
...
export function AppShell({ children }: { children: React.ReactNode }) {
    ...
    const filtered = searchItems.filter((item) => item.toLowerCase().includes(query.toLowerCase()));
    return <div className="min-h-screen"><Sidebar open={mobileOpen} onClose={() => setMobileOpen(false)} /><div className="lg:pl-[226px]">
        <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-zinc-200 bg-white/95 px-4 backdrop-blur md:px-6"><button onClick={() => setMobileOpen(true)} aria-label="Open navigation" className="rounded-md p-1.5 text-zinc-500 hover:bg-zinc-100 lg:hidden"><Menu className="h-5 w-5" /></button><button onClick={() => setSearchOpen(true)} className="flex h-8 w-full max-w-[340px] items-center gap-2 rounded-md border border-zinc-200 bg-zinc-50 px-2.5 text-xs text-zinc-500 transition hover:border-zinc-300 hover:bg-white"><Search className="h-3.5 w-3.5" /><span className="flex-1 text-left">Search anything...</span><kbd className="hidden rounded border border-zinc-200 bg-white px-1.5 py-0.5 text-[10px] text-zinc-400 sm:block">⌘ K</kbd></button><div className="ml-auto flex items-center gap-1.5">
            <div className="relative" ref={newRef}><button onClick={() => setNewOpen(!newOpen)} className="flex h-8 items-center gap-1.5 rounded-md bg-zinc-900 px-3 text-xs font-medium text-white shadow-sm hover:bg-zinc-800"><Plus className="h-3.5 w-3.5" /> New <ChevronDown className="h-3 w-3 text-zinc-400" /></button>{newOpen && <div className="animate-enter absolute right-0 top-10 w-48 rounded-lg border border-zinc-200 bg-white p-1.5 text-xs shadow-panel"><p className="px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Create</p>{newItems.map(({ icon: Icon, label }) => <button key={label} className="flex w-full items-center gap-2 rounded-md px-2 py-2 text-zinc-700 hover:bg-zinc-50"><Icon className="h-3.5 w-3.5 text-zinc-400" />{label}</button>)}</div>}</div>
            <div className="relative hidden sm:block" ref={envRef}><button onClick={() => setEnvironmentOpen(!environmentOpen)} className="flex h-8 items-center gap-2 rounded-md border border-zinc-200 bg-white px-2.5 text-xs font-medium text-zinc-700 hover:bg-zinc-50"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500 ring-2 ring-emerald-100" /> Production <ChevronDown className="h-3 w-3 text-zinc-400" /></button>{environmentOpen && <div className="animate-enter absolute right-0 top-10 w-48 rounded-lg border border-zinc-200 bg-white p-1.5 text-xs shadow-panel"><p className="px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-zinc-400">Environment</p><button className="flex w-full items-center justify-between rounded-md bg-zinc-50 px-2 py-2 font-medium"><span className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />Production</span><CheckCircle2 className="h-3.5 w-3.5" /></button><button className="mt-0.5 flex w-full items-center gap-2 rounded-md px-2 py-2 text-zinc-500 hover:bg-zinc-50"><span className="h-1.5 w-1.5 rounded-full bg-blue-400" />Staging</button></div>}</div>
            <button title="Notifications" aria-label="Notifications" className="relative grid h-8 w-8 place-items-center rounded-md text-zinc-500 hover:bg-zinc-100"><Bell className="h-4 w-4" /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-amber-500 ring-2 ring-white" /></button><button title="Profile" aria-label="Open profile" className="ml-0.5 grid h-8 w-8 place-items-center rounded-full bg-indigo-100 text-indigo-700 ring-1 ring-indigo-200"><UserRound className="h-4 w-4" /></button>
        </div></header><main className="mx-auto max-w-[1440px] p-4 md:p-6 lg:p-8">{children}</main></div>
        {searchOpen && <div className="fixed inset-0 z-50 flex justify-center bg-zinc-950/30 px-4 pt-[12vh] backdrop-blur-[2px]" onMouseDown={(e) => { if (e.target === e.currentTarget) setSearchOpen(false); }}><div className="animate-enter h-fit w-full max-w-xl overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-panel"><div className="flex items-center gap-3 border-b border-zinc-200 px-4"><Search className="h-4 w-4 text-zinc-400" /><input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search pipelines, sources, runs..." className="h-12 flex-1 bg-transparent text-sm outline-none placeholder:text-zinc-400" /><button onClick={() => setSearchOpen(false)} className="rounded border border-zinc-200 px-1.5 py-0.5 text-[10px] text-zinc-400">ESC</button></div><div className="p-2"><p className="px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-zinc-400">{query ? "Results" : "Recent"}</p>{filtered.length ? filtered.map((item, i) => <button key={item} onClick={() => setSearchOpen(false)} className="flex w-full items-center gap-3 rounded-lg px-2.5 py-2.5 text-left text-sm hover:bg-zinc-50"><Command className="h-4 w-4 text-zinc-400" /><span className="flex-1 font-medium text-zinc-700">{item}</span><span className="text-[11px] text-zinc-400">{i % 2 ? "Source" : "Pipeline"}</span></button>) : <p className="px-2 py-8 text-center text-sm text-zinc-500">No resources found.</p>}</div></div></div>}
    </div>;
}
