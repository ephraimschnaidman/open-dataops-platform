...
import { Activity, Bell, Boxes, CheckCircle2, ChevronDown, CircleGauge, Command, Database, FileCheck2, FileText, GitBranch, HeartPulse, LayoutDashboard, LogOut, Menu, MonitorDot, Plus, Search, Settings, ShieldAlert, Sparkles, X } from "lucide-react";
...
const searchItems = [
    ...
    { name: "run_01J94EVT18", type: "Run · Events Processing", href: "/pipeline-runs/run_01J94EVT18" },
    { name: "run_01J97BIL02", type: "Run · Billing Reconciliation", href: "/pipeline-runs/run_01J97BIL02" },
    { name: "run_01J92CING8", type: "Run · Customer Ingestion", href: "/pipeline-runs/run_01J92CING8" },
    { name: "run_01J92CVAL9", type: "Run · Customer Ingestion", href: "/pipeline-runs/run_01J92CVAL9" },
    { name: "run_01JA7OLD40", type: "Run · Customer Ingestion", href: "/pipeline-runs/run_01JA7OLD40" },
];
const newItems = [
...
function Sidebar({ open, onClose, currentEnvironment, onEnvironmentChange }: { open: boolean; onClose: () => void; currentEnvironment: string; onEnvironmentChange: (environment: (typeof environments)[number]) => void }) {
    ...
    return <><button aria-label="Close navigation" onClick={onClose} className={`fixed inset-0 z-30 bg-zinc-950/30 transition lg:hidden ${open ? "opacity-100" : "pointer-events-none opacity-0"}`} /><aside className={`fixed inset-y-0 left-0 z-40 flex w-[226px] flex-col border-r border-zinc-200 bg-[#fbfbfc] transition-transform duration-200 lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-14 items-center justify-between border-b border-zinc-200 px-4"><div className="flex items-center gap-2.5"><span className="grid h-7 w-7 place-items-center rounded-md bg-zinc-900 text-white"><CircleGauge className="h-4 w-4" /></span><span className="text-sm font-semibold tracking-[-0.02em]">Corvetra</span></div><button aria-label="Close navigation" onClick={onClose} className="rounded-md p-1 text-zinc-500 hover:bg-zinc-100 lg:hidden"><X className="h-4 w-4" /></button></div>
        <div className="border-b border-zinc-200 p-3 lg:hidden"><label className="block text-[10px] font-semibold uppercase tracking-[0.11em] text-zinc-400" htmlFor="mobile-environment">Environment</label><select id="mobile-environment" aria-label="Environment" value={currentEnvironment} onChange={(event) => onEnvironmentChange(event.target.value as (typeof environments)[number])} className="mt-2 h-9 w-full rounded-md border border-zinc-200 bg-white px-2.5 text-xs font-medium text-zinc-700">{environments.map((environment) => <option key={environment} value={environment}>{environment}</option>)}</select></div>
...
export function AppShell({ children }: { children: React.ReactNode }) {
    ...
    return <div className="min-h-screen"><Sidebar open={mobileOpen} onClose={() => setMobileOpen(false)} currentEnvironment={currentEnvironment} onEnvironmentChange={setCurrentEnvironment} /><div className="min-w-0 lg:pl-[226px]">
